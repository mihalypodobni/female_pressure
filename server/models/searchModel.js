...
const getGenresAndSubgenres = async function() {
	...
	for (let i = 0; i < genreList.length; i++) {
		if (
			genres.length === 0 ||
			genreList[i - 1].genre_name !== genreList[i].genre_name
		) {
			genres.push({
...
const getProfessionsAndSubProfessions = async function() {
	...
	for (let i = 0; i < professionList.length; i++) {
		if (
			...
			professionList[i - 1].profession_name !==
				professionList[i].profession_name
		) {
			professions.push({
...
const getCities = async function() {
	...
	for (let i = 0; i < cityList.length; i++) {
		...
		if (
			cities.length === 0 ||
			cityList[i - 1].continent_name !== cityList[i].continent_name
		) {
			cities.push({
		...
		if (
			stateY === -1 ||
			cityList[i - 1].country_name !== cityList[i].country_name
		) {
		...
		if (
			cities.length === 1 ||
			cityList[i - 1].state_long_name !== cityList[i].state_long_name
		) {
			cities[countryY].children[stateY].children.push({
				id:
					cityList[i].continent_name +
					";" +
					cityList[i].country_name +
					";" +
					cityList[i].state_long_name,
